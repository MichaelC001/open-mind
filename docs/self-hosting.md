...
**Privacy note:** uploaded JPEG, PNG, WebP, AVIF, and GIF images have EXIF/GPS, XMP, and IPTC metadata stripped losslessly on upload (pixel data is untouched — only metadata segments/chunks are removed). A malformed image (recognised type, but its bytes fail to parse) is rejected with `400 could not process image` rather than being stored.
